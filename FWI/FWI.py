...
for i in range(0, fwi_iterations):
    ...
    np.save('Results/%s/JMI+FWI-%s-vel_model%d'%(args.rdir, args.min_freq, i+1), model0.vp.data)

################################################################
######################### PLOT RESULTS #########################
np.save('Results/%s/JMI+FWI-%s-final_vel_model'%(args.rdir, args.min_freq), model0.vp.data)
